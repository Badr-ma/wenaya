/**
 * PanierView — shared basket content for the FR and EN /panier routes.
 * Locale-aware via useLocale (translations, hrefs, number format and the
 * article/item pluralization that previously differed per route page).
 * Hydration stays SSR-safe: renders a skeleton until the cart is hydrated.
 */
"use client";
...
export default function PanierView() {
  ...
  const itemWord = locale === "fr" ? "article" : "item";
  const itemWords = locale === "fr" ? "articles" : "items";
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#2B2F36]/35 mt-0.5">
                      {formatNumber(item.unitPrice, locale)} {item.currency} {t("panier.perUnit")}
                    </p>
                  </div>
                  ...
                  <p className="text-sm font-semibold text-[#0B1220] w-20 text-right shrink-0 tabular-nums">
                    {formatNumber(item.unitPrice * item.quantity, locale)} {item.currency}
                  </p>
            ...
            <div className="border-t border-[#0B1220]/[0.06] pt-6">
              <div className="flex justify-between items-baseline mb-2">
                <span className="text-sm text-[#2B2F36]/40">
                  {totalItems} {totalItems === 1 ? itemWord : itemWords}
                </span>
              ...
              <div className="flex justify-between items-baseline mb-6">
                ...
                <span className="text-xl font-bold text-[#0B1220]">
                  {formatNumber(subtotal, locale)} {items[0]?.currency || "MAD"}
                </span>
              </div>
