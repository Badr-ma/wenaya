...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${SITE_URL}/en/specialistes/${slug}`, languages: languageAlternates(`/specialistes/${slug}`) },
    openGraph: {
      ...OG_DEFAULTS,
      locale: "en_MA",
      title: `${specialist.name} — ${role}`,
      description,
      url: `${SITE_URL}/en/specialistes/${slug}`,
      type: "profile",
...
export default async function EnglishSpecialistPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    "@type": "Physician",
    "@id": `${SITE_URL}/en/specialistes/${slug}`,
