...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = [
    `${specialist.name}, ${role} in Casablanca.`,
    ...(specialist.yearsExperience > 0 ? [`${specialist.yearsExperience} years of experience.`] : []),
    ...(specialist.specialtyTags.length > 0 ? [`${specialist.specialtyTags.slice(0, 3).join(", ")}.`] : []),
    ...(specialist.rating > 0 ? [`Rating ${specialist.rating}/5 (${specialist.reviewCount} reviews).`] : []),
  ].join(" ");
