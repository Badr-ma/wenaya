/**
 * Product Detail Page — server component for individual product pages (/en/produits/[slug]).
...
export default async function EnglishProduitPage({ params }: Props) {
  const { slug } = await params;
  const product = getProductBySlug(slug, "en");
  if (!product) notFound();
